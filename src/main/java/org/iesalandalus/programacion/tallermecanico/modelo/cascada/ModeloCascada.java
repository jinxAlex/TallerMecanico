...
public class ModeloCascada implements Modelo {
    ...
    public void insertar(Trabajo trabajo) throws OperationNotSupportedException {
        ...
        if (trabajo instanceof Mecanico) {
            trabajoNuevo = new Mecanico(cliente,vehiculo,trabajo.getFechaInicio());
        } else if (trabajo instanceof Revision) {
            trabajoNuevo = new Revision(cliente,vehiculo,trabajo.getFechaInicio());
        }
        trabajos.insertar(trabajoNuevo);
    }

    @Override
    public Cliente buscar(Cliente cliente) {
        cliente = Objects.requireNonNull(clientes.buscar(cliente), "No existe un trabajo del cliente igual");
        return new Cliente(cliente);
    }

    @Override
    public Vehiculo buscar(Vehiculo vehiculo) {
        vehiculo = Objects.requireNonNull(vehiculos.buscar(vehiculo), "No existe un trabajo del vehículo igual");
        return vehiculo;
    }
    ...
    public void borrar(Vehiculo vehiculo) throws OperationNotSupportedException {
        List<Trabajo> coleccionVehiculos = trabajos.get(vehiculo);
        for (Trabajo trabajo : coleccionVehiculos) {
            trabajos.borrar(trabajo);
        }
    ...
    public List<Trabajo> getTrabajos(Vehiculo vehiculo) {
        ...
        for (Trabajo trabajo : trabajos.get(vehiculo)) {
            coleccionTrabajosVehiculo.add(Trabajo.copiar(trabajo));
        }
